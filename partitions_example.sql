...
INSERT INTO orders(account_id, client_id, items_price) VALUES (1, 2, 3);

-- Проверка - после выполнения в orders_3 ДОЛЖНА появится запись 
INSERT INTO orders(account_id, client_id, items_price) VALUES (2, 3, 4);
...
$$
    ...
    BEGIN
        LOOP
            DELETE
            FROM orders_1
            WHERE account_id = 1 AND id IN (SELECT id FROM orders_1 WHERE account_id = 1 LIMIT 1000);
